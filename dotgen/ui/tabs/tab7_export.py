"""Tab 7 -- Save job and export data.
...
class Tab7Export(QWidget):
    ...
    def _export(self) -> None:
        ...
        # Warnings, not errors: the export is legal, it is just likely to
        # produce a class with nothing in it.  Asked before the minutes are
        # spent rather than reported in the completion dialog after them.
        warnings = preflight_warnings(jobs, self.state.export)

        if warnings and QMessageBox.question(
            self,
            "Export data",
            "This export will probably ship an empty class:\n\n"
            + "\n".join("  - " + w for w in warnings)
            + "\n\nExport anyway?",
            QMessageBox.Ok | QMessageBox.Cancel,
            QMessageBox.Ok,
        ) != QMessageBox.Ok:
            self.statusMessage.emit("Export cancelled.")
            return
