...
class Tab5Class(QWidget):
    ...
    def _load(self) -> None:
        ...
        self.state.set_classes(
            build_classes(chars, self.state.lines, self.state.classes)
        )
    ...
    def refresh(self) -> None:
        ...
        errors = validate_classes(self.state.classes) if self.state.classes else ["No classes loaded yet."]

        if errors:
            self.validation.setText("\n".join("- " + e for e in errors))
            self.validation.setStyleSheet("color: #c82828;")
        else:
            self.validation.setText("All classes are valid. Tab 6 is unlocked.")
